...
public class PagingHandler {
	...
	public PagingHandler(PagingVO pgvo , int totalCount) {
		...
		// 컨트롤러에서 DB조회 훟 파라미터로 전송
		this.totalCount = totalCount ; // 총게시글의 갯수 
		...
		// 2/10 => 0.2*10 (1*10)0.2를 1로 변환. => 1*10
		// 11/10 => 1.1*10 (2*10) 1.1을 2로 변환. => 2*10
		// 정수 / 정수 => 정수 	/ 정수 / 실수 소수점이 나올수있게 둘중하나를 double로 형변환해주기
		this.endPage = (int)Math.ceil(pgvo.getPageNo() / (double)pgvo.getQty()) * pgvo.getQty();
		// 시작페이지네이션의 숫자
		this.startPage = this.endPage - 9 ;
				
		// 전체 게시글 수 / 한 화면에 게시되는 게시글 수
		// 101 * 10 => 10.1 => 올려서 11페이지가 되게해야한다.
		// 나머지 게시글이 하나라도 있으면 1페이지가 무조건 생겨야한다.
		this.realEndPage = (int)Math.ceil(totalCount / (double)pgvo.getQty()) ;
		
		//realEndPage = 11 / endPage = 20
		// 실제 페이지의 끝 설정
		// 진짜 끝 페이지가 endPage와 같지않을 경우 처리
