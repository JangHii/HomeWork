...
document.addEventListener('change',(e)=>{
    ...
    if(e.target.id == 'files'){ // 파일에 변화가 생겼다면..
        ...
        // ul > li로 값을 추가  
        // 여러 파일에 대한 검증을 모두 통과하기 위해서 곱하기(*)로 각 파일마다 통과 여부를 확인
        // 그래서 시작은 1로 해야한다. / 0을 곱하면 0이기때문
        let isOk = 1;
        let ul = `<ul class="list-group list-group-flush">`;
            for(let file of fileObj){ // fileObj은 배열로 
                // 개별 파일의 검증 결과
                ...
                console.log(vaildResult);
                    ...
                    ul += `${file.name}</div>`;
                    ul += `<span class="badge text-bg-${vaildResult ? '성공' : '실패'}">${file.size}</span>`;
                    ul += `</li>`;
            }
                ul += `</ul>`;
                div.innerHTML = ul;
